...
# TRAVERSAL FUNCTION
def dft(current_location = None, visited = None):
    ...
    if current_location is None:
        ...
        current_location = player.currentRoom.id
    ...
    print(f'Current Room ID: {current_location}')
    print(f'Visited List at Start: {visited}')

    # Stack to keep track of neighbors yet to be visited
    unvisited = []
    # For the length of available_direction
    while len(visited) < len(roomGraph) - 1:
        if len(stack) == 0:
            break

        # Get current location fron stack
        current_location = stack.pop()
        print(f'1. Stack after Pop: {stack}')

        # Add the current location to visited set
        if current_location not in visited:
            visited.append(current_location)
            # print(current_location)
            # stack.append(current_location)
        print(f'2. List of Visited in while: {visited}')

        # Get all available directions in current room
        available_directions = player.currentRoom.getExits()
        print(F'3. Current Room\'s available directions: {available_directions}')

        # Gets all directions from available rooms that have not been visited
        for item in available_directions:
            # Check to see if the ID of the room in that direction is in visited
            if roomGraph[int(current_location)][1].get(item) not in visited:
                unvisited.append(item)
                if roomGraph[int(current_location)][1].get(item) not in stack and roomGraph[int(current_location)][1].get(item) is not None:
                    stack.append(roomGraph[int(current_location)][1].get(item))
        print(f'4. Unvisited: {unvisited}')
        print(f'5. This is our Stack: {stack}')

        if len(unvisited) > 0:
            print('6. unvisited was not empty')
            # Grab a randon direction from available directions list
            direction = random.choices(unvisited)
            direction = ''.join(direction)
            print(f'7. The randomly chosen direction is: {direction}')
            # available_directions.remove(direction)
            chosen_room = roomGraph[int(current_location)][1].get(direction)
            print(f'8. Chosen Next Room is: {chosen_room}')

            if chosen_room not in visited:
                print('10. Chosen was not in visited')
                # Move player in to room in that direction
                player.travel(direction)
                # If true, call dft() and pass the visited set in as visited to the dft function
                traversalPath.append(direction)
                print(f'11. Traversal Path: {traversalPath}')
                dft(chosen_room, visited)

        # Convert available_directions to a string
        available_directions = ''.join(available_directions)

        # If we only have one direction available and that direction has already been visited
        if len(available_directions) == 1 and roomGraph[current_location][1].get(available_directions) in visited:
            print('6. Only one direction to travel and the direction was already visited')
            i = len(visited) - 1

            visited_list = tuple(visited) # change to tuple
            print(f'this is a list: {visited_list}')
            # test = []
            # for v in reversed(visited_list):
            #     print(f'We are in room: {v}')
            #     test.append(v)
            # print(f'Test is: {test}')
            last_in_stack = stack[len(stack)-1]
            print(f'last_in_stack: {last_in_stack}')
            # # graph at prev location in visited at index 1 at value of last traversal path
            # while visited_list[i] != last_in_stack:
            for key, value in roomGraph[visited_list[i]][1].items():
                if key == available_directions[0]:
                    print(f'7. We are going {key} to room {value}')
                    player.travel(key)
                    traversalPath.append(key)
                    print(f'8. Here is the value: {value}')
                # if v == stack[-1]:
                #     print(f'v equals: {v} and stack is: {stack[-1]}')
                #     break
            # while visited[i] != stack[-1]:
            #     # prev = roomGraph[visited[i]][1].items()
            #     for key, value in roomGraph[visited[i]][1].items():
            #         if key == available_directions[0]:
            #             print(f'We are going {key} to room {value}')
            #             player.travel(key)
            #             traversalPath.append(key)
        ...
        if len(unvisited) == 0:
            print('No unvited Neighbors')
            previous = stack.pop()
            print(f'Previous is: {previous} and stack is: {stack}')
            current_location = previous
            print(f'New Current Location: {current_location}')
            dft(current_location, visited)




        print(f'TraversalPath list: {traversalPath}')
